...
class PathSolver(AStar):
    def __init__(self, game: Game):
        """
        Initialize the path solver with the current game state.
        """
        self.game = game
        self.forbidden_cells: set[Cell] = set()

    def heuristic_cost_estimate(self, current: Cell, goal: Cell) -> float:
        """
        Estimate distance to the target using Manhattan distance.
        """
        return float(current.distance(goal))

    def distance_between(self, n1: Cell, n2: Cell) -> float:
        """
        Return movement cost between two neighboring cells.
        Forbidden cells receive infinite cost.
        """
    ...
    def neighbors(self, node: Cell) -> Iterable[Cell]:
        ...
        neighbors: list[Cell] = []
        if node.x > 0:
            neighbors.append(Cell(node.x - 1, node.y))
        if node.x < self.game.width - 1:
            neighbors.append(Cell(node.x + 1, node.y))
        if node.y > 0:
            neighbors.append(Cell(node.x, node.y - 1))
...
def next_step(game: Game) -> Cell | None:
    """
    Compute the best next step using A* pathfinding.

    The function evaluates valid food targets, forbidden cells,
    and available space after the move.
    """
    ...
    for x in range(width):
        forbidden_cells.add(Cell(x, 0))
        forbidden_cells.add(Cell(x, height - 1))
    for y in range(height):
        forbidden_cells.add(Cell(0, y))
        forbidden_cells.add(Cell(width - 1, y))
    ...
    for goal in goals:
        ...
        paths.append((first_step, length, area))
    ...
    if not paths:
        ...
        for cell in path_solver.neighbors(your_head):
            if cell in forbidden_cells:
                continue
            area = flood_fill_space(game, cell)
            if area > max_area:
                max_area = area
                next_cell = cell
    else:
